...
function collectionLabel(id, fallback = 'No Collection') { return id ? collectionById(id)?.name || 'Missing collection' : fallback; }
...
async function renderList(items) {
  const rows = await Promise.all(items.map(async (character) => {
    const cover = cardCover(character); const imageUrl = await loadImage(cover?.thumbnailPath || cover?.originalPath); const checked = state.selectedIds.has(character.id);
    return `<tr class="character-row ${checked ? 'selected' : ''}" data-action="open-editor" data-id="${character.id}"><td><input type="checkbox" data-select-id="${character.id}" ${checked ? 'checked' : ''} /></td><td>${imageUrl ? `<img class="list-thumb" src="${imageUrl}" alt="" />` : `<span class="list-initial">${escapeHtml(character.title.slice(0, 1).toUpperCase() || 'C')}</span>`}</td><td><strong>${escapeHtml(character.title)}</strong><small>${escapeHtml(character.subtitle || 'No subtitle')}</small></td><td>${chipMarkup(character.tags.slice(0, 3))}</td><td>${chipMarkup(character.compatibleModels.slice(0, 2), 'model-chip')}</td><td>${escapeHtml(character.collectionId ? collectionLabel(character.collectionId) : '—')}</td><td>${character.scenes.length}</td><td>${character.gallery.length}</td><td>${formatDate(character.updatedAt)}</td><td>${character.favorite ? '★' : ''} ${character.archived ? 'Archived' : ''}</td><td><button data-action="copy-system" data-id="${character.id}">Copy</button></td></tr>`;
  }));
...
function currentCollectionHeader() {
  ...
  if (state.filters.collection === 'unfiled') return `<section class="filter-notice"><span>No Collection</span><p>Showing characters that are not assigned to a collection.</p></section>`;
...
function collectionSelect(selectedId) { return `<select data-field="collectionId"><option value="">No Collection</option>${state.collections.map((collection) => `<option value="${collection.id}" ${selectedId === collection.id ? 'selected' : ''}>${escapeHtml(collection.name)}</option>`).join('')}</select>`; }
...
async function render() {
  const app = $('#app');
  const showNoCollection = state.collections.length > 0 || state.filters.collection === 'unfiled';
  app.innerHTML = `<aside class="sidebar"><div class="brand"><span>CK</span><strong>CharacterKeep</strong></div><nav><button class="${state.view === 'characters' && state.filters.collection === 'all' ? 'active' : ''}" data-nav="characters" data-collection="all">All Characters</button>${showNoCollection ? `<button class="${state.view === 'characters' && state.filters.collection === 'unfiled' ? 'active' : ''}" data-nav="characters" data-collection="unfiled">No Collection</button>` : ''}<div class="sidebar-section"><div class="sidebar-label"><span>Collections</span><button class="mini-add" data-action="new-collection" aria-label="New collection">+</button></div>${state.collections.map((collection) => `<button class="nav-subitem ${state.filters.collection === collection.id ? 'active' : ''}" data-nav="characters" data-collection="${collection.id}">${escapeHtml(collection.name)}</button>`).join('')}</div><button class="${state.view === 'help' ? 'active' : ''}" data-nav="help">Help</button><button class="${state.view === 'settings' ? 'active' : ''}" data-nav="settings">Settings</button></nav></aside>${state.view === 'settings' ? settingsView() : state.view === 'help' ? helpView() : await renderCharactersView()}${state.draft ? await editorView() : ''}${createCharacterDialog()}${exportDialog()}${preflightDialog()}${importResultDialog()}`;
}
