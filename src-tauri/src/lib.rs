...
fn generate_thumbnail(
    ...
    if let Some(parent) = thumb_path.parent() {
        fs::create_dir_all(parent).map_err(|e| format!("Failed to create thumbnail directory: {}", e))?;
    }
...
fn create_characterkeep_backup_zip(app: tauri::AppHandle, dest_path: String) -> Result<(), String> {
    ...
    if let Some(items) = characters.as_array() {
        for character in items {
            ...
            write_json_entry(&mut writer, &format!("characters/{}.json", id), character, options)?;
        }
    }

    if media_root.exists() {
        for entry in WalkDir::new(&media_root).into_iter().filter_map(|entry| entry.ok()) {
...
#[tauri::command]
fn restore_characterkeep_backup_zip(app: tauri::AppHandle, source_path: String) -> Result<RestoreSummary, String> {
    ...
    let mut current = current_value.as_array().cloned().unwrap_or_default();
    ...
    for mut character in incoming {
        ...
        let mut character_media = 0usize;
        for (path, bytes) in &media {
            if !path.starts_with(&media_prefix) {
                continue;
            }
            let dest_rel = path.replacen(&media_prefix, &imported_prefix, 1);
            let dest = data_dir.join(resolve_relative_path(&dest_rel)?);
            if let Some(parent) = dest.parent() {
        ...
        if character.get("gallery").and_then(|v| v.as_array()).map(|g| !g.is_empty()).unwrap_or(false) && character_media == 0 {
            summary.missing_media += 1;
        }

        hashes.insert(stable_hash(&character));
        if let Some(id) = value_id(&character) {
            id_to_hash.insert(id, incoming_hash);
        }
        current.push(character);
        summary.imported += 1;
    }

    fs::write(&characters_path, serde_json::to_string_pretty(&current).map_err(|e| e.to_string())?)
